...
class ManipulationModelling():
    ...
    def auto_pairwise_keep_dist(self, time_interval, objs_lst1, margin = 0.001):
        """
    ...
    def target_relative_xy_position(self, time, obj, relativeTo, pos):
        ...
        if len(pos)==2:
            pos.append(0.)
        self.komo.addObjective([time], ry.FS.positionRel, [obj, relativeTo], ry.OT.eq, scale=1e3*np.array([[1,0,0],[0,1,0]]), target=pos)
        #self.komo.addObjective([time], ry.FS.positionDiff, [obj, relativeTo], ry.OT.eq, scale=1e1*np.array([[1,0,0],[0,1,0]]), target=pos)
